...
class MaximWindow(QMainWindow):
    ...
    def _capture_handshake(self, essid):
        ...
        # Step 2: Scan — use --essid filter to find exact network (up to 5 min)
        ...
        # Use --essid to filter for exact ESSID — guarantees full name match
        script.write(f"  sudo timeout 30 airodump-ng --essid '{essid}' -w '{scan_file}' --output-format csv --write-interval 1 $MON &\n")
